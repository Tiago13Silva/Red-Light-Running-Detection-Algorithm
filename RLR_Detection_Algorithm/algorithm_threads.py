...
class AlgorithmWithThreads:
    """
    This class is a paralel version of the proposed Algorithm with the difference of using threads for
    processing the RLR detections.
    Each pair of regions and its RLR detection is processed by one thread, with at most 6 threads/pairs.
    To maintain the coordination of the threads, Queues are used for data transfering between threads and
    to wait each thread termination.
    """
    ...
    def start_region_threads(self):
        ...
        if len(self.threads) > 6 :
            print("The algorithm does not support more than 6 pairs of regions!")
            raise SyntaxError(f"The algorithm does not support more than 6 pairs of regions!")
